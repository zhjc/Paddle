...
class TransposeOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    int dims = input->GetDimensions().Size();
    std::vector<int> axis =
    ...
    AscendIE::PermutationConf perm;
    for (int i = 0; i < dims; i++) {
      int j = engine_->with_dynamic_shape() ? i : i + 1;
      perm.order[i] = axis[j];
    }
    // auto* layer = TRT_ENGINE_ADD_LAYER(engine_, Shuffle, *input);
    auto* layer = engine_->network()->AddShuffle(input);
...
REGISTER_ASCEND_OP_CONVERTER(transpose, TransposeOpConverter);
REGISTER_ASCEND_OP_CONVERTER(transpose2, TransposeOpConverter);
