...
/*
 * Help to validate the correctness between Fluid Op and the corresponding AIE
 * layer.
 */
class AscendIEConvertValidation {
 public:
  AscendIEConvertValidation() = delete;

  AscendIEConvertValidation(int max_batch_size,
    ...
    PADDLE_ENFORCE_EQ((int)AscendIE::CreateStream(&stream_),
                      0,
                      platform::errors::External("cudaStreamCreate error."));
    engine_.reset(new AscendIEEngine(max_batch_size, workspace_size));
    engine_->InitNetwork();
  }
  ...
  void DeclInputVar(const std::string& name,
                    ...
                    const AscendIE::Dims& trt_dims) {
    DeclVar(name, tensor_dims);
    engine_->DeclareInput(name, AscendIE::DataType::FLOAT, trt_dims);
  }

  void DeclInputVar(const std::string& name, const AscendIE::Dims& dims) {
    DeclVar(name, dims);
    // Declare TRT inputs.
    engine_->DeclareInput(name, AscendIE::DataType::FLOAT, dims);
  }
  ...
  void DeclParamVar(const std::string& name, const AscendIE::Dims& dims) {
  ...
  void DeclOutputVar(const std::string& name, const AscendIE::Dims& dims) {
    DeclVar(name, dims);
  }
  ...
  void DeclVar(const std::string& name, const std::vector<int> dim_vec) {
    ...
    RandomizeTensor(x_tensor, place_);
  }
  ...
  void DeclVar(const std::string& name,
               ...
               bool is_param = false) {
    // Init Fluid tensor.
    std::vector<int> dim_vec(&dims[0], &dims[0] + dims.Size());
  ...
  void Execute(int batch_size,
    ...
    AscendIE::SynchronizeStream(stream_);
    ...
    for (const auto& output : op_desc_->OutputArgumentNames()) {
      ...
      framework::TensorToVector(*tensor, &fluid_out);
    ...
    for (const auto& output : op_desc_->OutputArgumentNames()) {
      ...
      auto* var = scope_.FindVar(output);
      auto* tensor = var->GetMutable<phi::DenseTensor>();
      framework::TensorToVector(*tensor, &trt_out);
 ...
 private:
  ...
  std::unique_ptr<AscendIEEngine> engine_;
  AscendIE::aieStream stream_;
