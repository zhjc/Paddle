...
class FillAnyLikeOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    AscendIE::Tensor* input = engine_->GetITensor(op_desc.Input("X").front());
    auto output_name = op_desc.Output("Out").front();
    AscendIE::Dims input_dims = input->GetDimensions();
    auto nbDims_num = input_dims.Size();
    AscendIE::Tensor* value_tensor;

    phi::ProtoDataType dtype =  static_cast<phi::ProtoDataType>(PADDLE_GET_CONST(int, op_desc.GetAttr("dtype")));
    float value = PADDLE_GET_CONST(float, op_desc.GetAttr("value"));
    if ((dtype == phi::ProtoDataType::INT32) ||
        (dtype == -1 && input->GetType() == AscendIE::DataType::INT32)) {
    ...
    AscendIE::Tensor* one_rank_tensor = Add1DConstantLayer(
        std::vector<int32_t>(nbDims_num, 1), output_name + "_one_rank_tensor_");
    AscendIE::Tensor* input_shape_tensor = one_rank_tensor;
    auto* shuffle = engine_->network()->AddShuffle(value_tensor);
    shuffle->SetInput(1, input_shape_tensor);

    std::vector<int32_t> start_vec(nbDims_num, 0);
    AscendIE::Dims start(nbDims_num, (int64_t*)&start_vec[0]);
    AscendIE::Dims size(start);
    AscendIE::Dims stride(start);
    ...
    auto layer = engine_->network()->AddSlice(shuffle->GetOutput(0), start, size, stride);
...
REGISTER_ASCEND_OP_CONVERTER(fill_any_like, FillAnyLikeOpConverter);
