...
class TopKOpConverter : public OpConverter {
 public:
  ...
  void operator()(const framework::proto::OpDesc& op,
    ...
    VLOG(3) << "convert a top_k op to ascendie TopK layer";
    ...
    AscendIE::Dims input_dims = input_tensor->GetDimensions();
    int axis = input_dims.Size();
    AscendIE::TopKLayer* layer = engine_->network()->AddTopK(input_tensor,
                                                              AscendIE::TopKOperation::MAX,
                                                              k,
                                                              {1 << (axis - 1)});
  ...
  void operator()(const framework::proto::OpDesc& op,
    ...
    const bool largest =
        op_desc.HasAttr("largest")
            ...
            : true;
    auto flag =
        largest ? AscendIE::TopKOperation::MAX : AscendIE::TopKOperation::MIN;
    AscendIE::TopKLayer* layer = nullptr;
    if (axis == -1) {
      AscendIE::Dims input_dims = input_tensor->GetDimensions();
      layer = engine_->network()->AddTopK(input_tensor, flag, k, {1 << (input_dims.Size() - 1)});
    } else {
      if (engine_->with_dynamic_shape()) {
        layer = engine_->network()->AddTopK(input_tensor, flag, k, {1 << axis});
      } else {
        layer = engine_->network()->AddTopK(input_tensor, flag, k, {1 << (axis - 1)});
      }
    }
    std::vector<std::string> output_names;
    output_names.push_back(op_desc.Output("Out").front());
...
REGISTER_ASCEND_OP_CONVERTER(top_k, TopKOpConverter);
REGISTER_ASCEND_OP_CONVERTER(top_k_v2, TopKv2OpConverter);
